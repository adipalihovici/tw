...
var localProfile;
  passport.use('facebook', new FacebookStrategy( {
    ...
    scope: ["publish_actions"]
        ...
        db.Users.update({"fid" : profile._json.id},{"name": profile._json.name, "first_name": profile._json.first_name, "last_name": profile._json.last_name, "gender" : profile._json.gender, "photo": profile._json.picture.data.url, accessToken: accessToken, refreshToken: refreshToken},{upsert:true});
        done(null, profile);
}));
...
updateDatabaseGameOver = function(room){ ///////// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  console.log('Update in DB ' + room.name);
      ...
      stats.update({},{ $inc:{ Games : 1}}, function (err, result){
        if (err) {
            console.log(err);
          }
          else{
            ...
            collection.update({"_id" : room.players[i].playerData.id},{ $inc: {"gamecount" :1, "points": room.players[i].score} , $set: {level: Math.floor((value.gamecount+1)/10)}}, function (err, result){
              if (err) {
                ...
                console.log("Updated player: " + room.players.playerData.id);
              }
          });
          }}});
      //var pts = collection.findOne({"_id" : room.players[i].playerData.id})
    }}
...
function questionsForRoomWithLevel(howMany, level){
  //return questionDB;
    var Q = db.collection('Questions'); 
    var questions = Q.find({diff: level},{});
    questions.then(function(value){
     // value.sort({asked: 1});
      value.limit(howMany).toArray();
      return value;
    });
}
...
function getLeader(howMany){

  var cursor = db.Users.find({}).sort({points : -1}).toArray(function(err,result){
        if(err){
          res.send(err);
        } else if (result.length){
          var first = result.slice(howMany-1);
          return first;
        } else{
...
function myStats(id){
  var info = db.Users.findOne({"fid": id});
  info.then(function(value){ return value;});
}
